...
Вывод команды **show port-security interface** проверяет включена ли функция port security, существует ли хост, подключенный к порту (т.е., Secure-up), всем 2 MAC адреса будет разрешено подключиться, и S1 выучил один MAC адрес статически и один MAC адрес динамически (т.е., sticky).

Вывод команды **show port-security address** перечисляет выученных два MAC адреса.
...
[Открыть описание в PDF](./assets/11.1.10-packet-tracer---implement-port-security_ru-RU.pdf)

[Скачать файл для Packet Tracer](./assets/11.1.10-packet-tracer---implement-port-security_ru-RU.pka)
