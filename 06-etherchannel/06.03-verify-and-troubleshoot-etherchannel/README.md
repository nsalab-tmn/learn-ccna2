...
Как всегда, при настройке устройств в сети необходимо проверить конфигурацию. Если есть проблемы, вы должны иметь возможность обнаружить и устранить их. В этом разделе приведены команды для проверки, а также некоторые распространенные проблемы сети EtherChannel и их решения.
...
Группа интерфейсов состоит из интерфейсов FastEthernet0/1 и FastEthernet0/2. Она является каналом EtherChannel второго уровня и задействована, на что указывают буквы SU рядом с номером агрегированного канала.
...
Чтобы просмотреть данные о роли физического интерфейса в работе EtherChannel, следует выполнить команду **show interfaces etherchannel**. Интерфейс FastEthernet0/1 относится к группе EtherChannel 1. Для данного канала EtherChannel используется протокол LACP.
...
Все интерфейсы в EtherChannel должны иметь одинаковые настройки скорости и дуплексного режима, одинаковые VLAN с нетегированным трафиком, разрешенные VLAN в магистралях и VLAN доступа на портах. Обеспечение этих конфигураций значительно снизит сетевые проблемы, связанные с EtherChannel. Есть несколько распространенных проблем EtherChannel.

* Назначенные порты в EtherChannel не являются частью одной VLAN или не настроены как транки. Порты с разными Native VLAN не могут образовать EtherChannel.
...
**Примечание.** Легко спутать PAgP или LACP с DTP, поскольку эти протокола\ы используются для автоматизации поведения на транковых каналах. PAgP и LACP нужны для агрегирования каналов (EtherChannel). DTP используется для автоматизации создания транковых каналов. Как правило, если готов транковый канал, EtherChannel (PAgP или LACP) настраивается в первую очередь, и только после этого приходит очередь DTP.
...
**Шаг 1. Просмотрите сводную информацию EtherChannel**
...
**Шаг 2. Просмотрите конфигурацию Port Channel**

В выводе **show run | begin interface port-channel** представлены более подробные выходные данные, в которых указано, что на коммутаторах S1 и S2 настроены несовместимые режимы PAgP.
...
**Шаг 3. Исправьте все неправильно выполненные настройки**
...
**Шаг 4. Проверьте работоспособность EtherChannel**

EtherChannel теперь активен, как проверено выходными данными команды **show etherchannel summary**.
