...
Часть 1. Настроить основное сетевое устройство.

Часть 2. Настроить VLAN

Часть 3. Настроить безопасность коммутатора.
...
2.  Активируйте устройства.

**Шаг 2. Настройте роутер R1**
...
**Шаг 1. Создайте VLAN 10**

Добавьте VLAN 10 на S1 и S2 и назовите ее **Management.**

**Шаг 2. Создайте SVI для VLAN 10**
...
Интерфейсы F0/6 на S1 и F0/18 на S2 настроены как порты доступа. На этом шаге вы также настроите безопасность на этих двух портах доступа.
...
2.  На S1 включите защиту порта на F0/6 с нижеперечисленными настройками.
...
4.  Включите безопасность порта для F0/18 на S2. Настройте каждый активный порт доступа таким образом, чтобы он автоматически добавлял МАС-адреса, изученные на этом порту, в текущую конфигурацию.
5.  Настройте параметры безопасности порта на S2 F0/18.
...
1.  На S2 включите DHCP-snooping и настройте его во VLAN 10.
2.  Настройте магистральные порты на S2 как доверенные порты.
3.  Ограничьте ненадежный порт F0/18 на S2 пятью DHCP-пакетами в секунду.
...
2.  Что касается безопасности порта на S2, если вы загружаете скрипт текущей конфигурации, почему порту 18 на PC-B никогда не получит IP-адрес через DHCP?
