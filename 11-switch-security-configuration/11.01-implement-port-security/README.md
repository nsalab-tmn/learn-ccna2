...
Устройства уровня 2 считаются самым слабым звеном в инфраструктуре безопасности компании. Атаки второго уровня являются одними из самых простых для развертывания хакерами, но эти угрозы также можно нейтрализовать с помощью некоторых распространенных решений этого же уровня.

Перед введением коммутатора в эксплуатацию необходимо обеспечить безопасность всех портов (интерфейсов) коммутатора. Как каждый из них будет защищен, зависит от их функций.

**Отключение неиспользуемых портов** — это простой способ защиты сети от несанкционированного доступа, используемый многими администраторами. К примеру, если коммутатор Catalyst 2960 имеет 24 порта и при этом используются три подключения Fast Ethernet, рекомендуется отключить 21 неиспользуемый порт. Перейдите к каждому и выполните команду выключения Cisco IOS **shutdown**. Если позднее порт необходимо будет активировать, его можно включить с помощью команды **no shutdown**.

Чтобы настроить отключение для целого диапазона портов, используйте команду **interface range**.

```
Switch(config)# interface range type module/first-number – last-number 
```

Например, чтобы отключить порты от Fa0/8 до Fa0/24 на S1, вы должны ввести следующую команду:
...
Данная функция ограничивает количество допустимых МАС-адресов на один порт, а также это позволяет администратору вручную настраивать MAC-адреса для порта или разрешать коммутатору динамически изучать ограниченное количество адресов. Когда порт, сконфигурированный с защитой порта, получает кадр, MAC-адрес источника сравнивается со списком адресов защищенного источника, которые были настроены или динамически изучены на порту.

Ограничив число разрешенных MAC-адресов одним, можно использовать средства безопасности портов для контроля несанкционированного расширения сети, как показано на рисунке.

![](./assets/11.1.2.svg)


**Примечание.** MAC-адреса показаны как 24-битные для простоты.

<!-- 11.1.3 -->
## Включите защиту портов

Обратите внимание, что в примере команда **switchport port-security** была отклонена. Это связано с тем, что безопасность портов можно установить только на настроенных вручную портах доступа или магистральных портах. По умолчанию порты коммутатора уровня 2 настроены на динамический автоматический режим (транкинг включен). Поэтому в примере порт настраивается с помощью команды настройки интерфейса **switchport mode access**.
...
Используйте команду **show port-security interface** для отображения текущих настроек безопасности порта для FastEthernet 0/1, как показано в примере. Обратите внимание на то, как включена защита порта: режим нарушения (violation mode) отключен, максимальное количество MAC-адресов равно 1. Если устройство подключено к порту, коммутатор автоматически добавит его MAC-адре в качестве защищенного. В этом примере ни одно устройство не подключено к порту.
...
**Примечание.** Если активный порт настроен с помощью команды **switchport port-security** и к нему подключено более одного устройства, он перейдет в состояние error-disabled. Это условие обсуждается позже в этой теме.

После включения защиты порта можно настроить другие особенности его безопасности, как показано в примере.
...
В следующем примере демонстрируется полная настройка безопасности порта для FastEthernet 0/1. Администратор указывает максимум 2 MAC-адреса, вручную настраивает один безопасный, а затем настраивает порт для динамического изучения дополнительных защищенных MAC-адресов (до максимум 2). Используйте команды **show port-security interface** и **show port-security address** для проверки конфигурации.
...
Вывод команды **show port-security interface** проверяет, включена ли функция port security, существует ли хост, подключенный к порту (т.е., Secure-up), обоим MAC-адресам будет разрешено подключиться, и S1 выучил один адрес статически и один динамически (т.е., sticky).
...
| **Параметр**  | **Описание**  |
| --- | --- |
| `static` | Включает устаревание для статически настроенных безопасных адресов на порту |
| `time time ` | Определяет время устаревания для порта (диапазон составляет от 0 до 1440 минут, сли время равно 0, устаревание для этого порта отключено) |
| `type absolute` | Устанавливает абсолютное время устаревания (срок действия всех защищенных адресов на этом порту истекает точно по окончании указанного в минутах времени и удаляется из списка защищенных адресов) |
...
Далее в таблицах показано, как коммутатор реагирует в зависимости от настроенного режима нарушения.

**Описание режимов Security Violation**

| **Режим** | **Описание** |
| --- | --- |
| `shutdown` (default) | Порт сразу переходит в состояние отключения по ошибке, гаснет светодиод порта и отправляется сообщение системного журнала. Увеличивается счетчик нарушений нарушений. Когда защищенный порт находится в состоянии отключения по ошибке, администратор должен повторно включить его, используя команы **shutdown** и **no shutdown**. |
| `restrict` (ограничение) | Порт отбрасывает пакеты с неизвестными адресами источника, пока вы не удалите достаточное количество защищенных MAC-адресов, чтобы опуститься ниже максимального значения или увеличить его. Этот режим вызывает увеличение счетчика нарушений безопасности и генерирует сообщение системного журнала (syslog). |
| `protect` (защита) | Это наименее безопасный из режимов нарушения безопасности. Порт так же отбрасывает пакеты с неизвестными адресами источника, пока вы не удалите достаточное количество защищенных MAC-адресов, чтобы опуститься ниже максимального значения или увеличить его, но сообщений системного журнала (syslog) нет. |
...
Когда порт отключен и переведен в состояние error-disabled, трафик не отправляется и не принимается. На консоли отображается ряд сообщений, связанных с безопасностью портов, как показано в следующем примере.
...
Сначала используйте команду **shutdown**, затем — команду **no shutdown**, чтобы сделать порт работоспособным, как показано в примере.
