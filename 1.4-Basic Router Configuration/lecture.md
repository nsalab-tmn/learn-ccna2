...
<!-- 1.4.2 -->
## Проверка синтаксиса - настройка основных параметров маршрутизатора
...
- **Активировать** - By default, LAN and WAN interfaces are not activated ( **с использованием команды** ). To enable an interface, it must be activated using the **no shutdown**. (**Это действие аналогично подаче питания на интерфейс.**) Для активации физического уровня интерфейс должен быть также подключен к другому устройству (концентратору, коммутатору или другому маршрутизатору).
...
<!-- 1.4.5 -->
## Проверка синтаксиса - настройка интерфейсов маршрутизатора
