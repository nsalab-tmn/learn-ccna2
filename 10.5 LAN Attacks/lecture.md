...
Нажмите Воспроизвести на рисунке, чтобы просмотреть видео о атаках VLAN и DHCP.
...
- vУбедитесь, что native VLAN используется только для магистральных каналов.
...
**Злоумышленник подключает подставной DHCP-сервер**
...
**Клиент делает широковещательную рассылку сообщений об обнаружении DHCP**
...
**Законный и мошеннический ответ DHCP**
...
**Клиент принимает мошенническое предложение DHCP**
...
**Подставной сервер подтверждает запрос.**
...
Нажмите Play на рисунке, чтобы просмотреть видео о атаках VLAN и DHCP.
...
**Нормальное состояние с конвергентными таблицами MAC**
...
- Первый информирует все устройства в локальной сети о том, что MAC-адрес злоумышленника (CC: CC: CC) сопоставляется с IP-адресом R1, 10.0.0.1.
- Второй информирует все устройства в локальной сети о том, что MAC-адрес злоумышленника (CC: CC: CC) сопоставляется с IP-адресом ПК1, 10.0.0.11.

![](./assets/10.5.7-2.png)

**Шаг 3**
**Атака «отравление» ARP-кэша с атакой "человек посередине"**
