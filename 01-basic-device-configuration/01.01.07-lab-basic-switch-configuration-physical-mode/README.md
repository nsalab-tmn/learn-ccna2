...
На данном этапе вам нужно проверить такие параметры коммутатора по умолчанию, как его текущие настройки, данные IOS, свойства интерфейса, сведения о VLAN и флеш-память.

Вы можете получить доступ ко всем командам коммутатора IOS в привилегированном режиме EXEC. Его нужно ограничить с помощью пароля, чтобы предотвратить неавторизованное использование устройства — через этот режим можно получить прямой доступ к режиму глобальной конфигурации и командам, необходимым для настройки рабочих параметров. Вы установите пароли позже в этом задании.

К привилегированному набору команд относятся команды пользовательского режима, а также **configure**, при помощи которой выполняется доступ к остальным режимам. Введите команду **enable**, чтобы войти в привилегированный режим EXEC.

1.  Предполагая, что у коммутатора нет файла конфигурации, хранящегося в энергонезависимой памяти (NVRAM), при консольном подключении с использованием программы **terminal** вы перейдете в пользовательский режим EXEC на коммутаторе с приглашением **Switch\>**. Введите команду **enable**, чтобы войти в привилегированный режим EXEC.
...
2.  Назначьте IP-адрес интерфейсу SVI на коммутаторе. Благодаря этому вы получите возможность удаленно управлять коммутатором.

    Прежде чем вы сможете управлять коммутатором **S1** удаленно с компьютера **PC-A**, коммутатору нужно назначить IP-адрес. Согласно конфигурации по умолчанию им можно управлять через VLAN 1. Однако в базовой конфигурации коммутатора не рекомендуется назначать VLAN 1 в качестве административной VLAN.

    Для административных целей используйте VLAN 99. Такой выбор является случайным, поэтому вы не обязаны использовать эту сеть всегда.

    Итак, для начала создайте на коммутаторе новую VLAN 99. Затем настройте IP-адрес коммутатора на 192.168.1.2 с маской подсети 255.255.255.0 на внутреннем виртуальном интерфейсе (SVI) VLAN 99. IPv6-адрес также можно настроить на интерфейсе SVI. Настройте IPv6-адреса для роутеров в соответствии с **таблицей адресации**.
...
5.  Настройте шлюз по умолчанию для коммутатора **S1**. Если не настроен ни один шлюз по умолчанию, коммутатором нельзя управлять из удаленной сети, на пути к которой имеется более одного роутера. Хотя в этом упражнении не учитывается внешний IP-шлюз, представьте, что впоследствии вы подключите LAN к роутеру для обеспечения внешнего доступа. При условии, что интерфейс LAN роутера равен 192.168.1.1, настройте шлюз по умолчанию для коммутатора.
...
Назначьте компьютеру IP-адрес и маску подсети в соответствии с **таблицей адресации**. Здесь описана сокращенная версия данной операции. Для рассматриваемой топологии не требуется шлюз по умолчанию. Однако вы можете ввести адрес **192.168.1.1** и **fe80::1**, чтобы смоделировать роутер, подключенный к коммутатору **S1**.

1.  Перейдите во вкладку **Desktop**.

2.  Щелкните **IP Configuration**.

3.  Убедитесь, что выбрана радио-кнопка **Static** в разделе IP Configuration.

4.  Введите IPv4-адрес, маску подсети и шлюз по умолчанию.

5.  Убедитесь, что выбрана радио-кнопка **Static** в разделе IPv6 Configuration.

6.  Введите IPv6-адрес, префикс и шлюз по умолчанию

7.  Нажав **Х**, закройте окно **IP Configuration**.
