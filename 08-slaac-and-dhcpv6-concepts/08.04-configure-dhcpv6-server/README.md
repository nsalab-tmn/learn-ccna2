...
Роутеры Cisco IOS являются мощными устройствами. В небольших сетях нет необходимости иметь отдельные устройства, чтобы иметь DHCPv6 сервер, клиент или агент ретрансляции. Роутер Cisco можно настроить для предоставления служб DHCPv6.

В частности, он может быть настроен следующим образом:

* **DHCPv6 сервер** (предоставляет службы DHCPv6 с сохранением состояния или без);
* **DHCPv6-клиент** (интерфейс получает конфигурацию IPv6 IP с сервера DHCPv6);
* **DHCPv6 Relay Agent** (предоставляет услуги переадресации DHCPv6, когда клиент и сервер находятся в разных сетях).

<!-- 8.4.2 -->
## Настройка роутера в качестве DHCPv6-сервера без отслеживания состояния

Параметр DHCPv6 сервера без учета состояния требует, чтобы устройство объявляло информацию о сетевой IPv6-адресации в сообщениях RA. Тем не менее клиент должен обратиться к DHCPv6-серверу для получения дополнительных сведений.

Посмотрите пример топологии, чтобы узнать, как настроить метод сервера DHCPv6 без сохранения состояния.
...
Для настройки и проверки роутера как DHCPv6 без состояния сервера DHCPv6 необходимо выполнить пять шагов.
...
Для активации IPv6-маршрутизации необходимо выполнить команду **ipv6 unicast-routing**. Это не является необходимым для настройки роутера в качестве DHCPv6-сервера без отслеживания состояния, но требуется для устройства, чтобы отправлять сообщения RA по протоколу ICMPv6.
...
Создайте пул DHCPv6 с помощью команды глобальной конфигурации **ipv6 dhcp pool _POOL-NAME_**. Это входит в режим подконфигурации пула DHCPv6, как указано в **Router(config-dhcpv6)#**.
...
Обратите внимание на вывод, что PC1 создал свой GUA IPv6 с помощью префикса 2001:db8:acad:1::/64. Также важно, что шлюзом по умолчанию является локальный адрес канала IPv6 R1. Это подтверждает, что PC1 получает конфигурацию IPv6 из RA R1.
...
Роутер также может быть клиентом DHCPv6 и получать конфигурацию IPv6 с сервера DHCPv6. Например, тот, который функционирующий в качестве сервера DHCPv6. На рисунке R1 в качестве DHCPv6-сервера без отслеживания состояния.

![](./assets/8.4.3.svg)


Для настройки и проверки роутера как DHCPv6 без сохранения состояния сервера DHCPv6 необходимо выполнить пять шагов.

**Шаг 1**. Включите маршрутизацию IPv6.  
**Шаг 2**. Настройте клиентский роутера для создания LLA.  
**Шаг 3**. Настройте клиентский роутер на использование SLAAC.  
**Шаг 4**. Убедитесь, что клиентскому роутеру назначен GUA.  
**Шаг 5**. Убедитесь, что клиентский роутер получил другую необходимую информацию DHCPv6.

**Шаг 1. Включите маршрутизацию IPv6**

На клиентском устройстве DHCPv6 должна быть включена команда **ipv6 unicast-routing**.
...
**Шаг 2. Настройте клиентский роутер для создания LLA**

Роутер должен иметь локальный адрес канала IPv6, который создается на интерфейсе устройства при настройке глобального одноадресного адреса. Он также может быть создан без GUA с помощью команды конфигурации интерфейса **ipv6 enable**. Cisco IOS использует EUI-64 для создания случайного идентификатора интерфейса.

В примере команда **ipv6 enable** настраивается на интерфейсе Gigabit Ethernet 0/0/1 клиентского роутера R3.
...
Клиентское устройство должно быть настроено на использование SLAAC для создания конфигурации IPv6. Команда **ipv6 address autoconfig** включает автоматическую настройку IPv6-адресации с использованием SLAAC.
...
**Шаг 5. Убедитесь, что клиентский роутер получил другую необходимую информацию DHCPv6**
 ...
 Команда **show ipv6 dhcp interface g0/0/1** подтверждает, что DNS-имена и доменные имена также были изучены устройством R3.
...
<!-- 8.4.4 -->
## Настройка роутера в качестве сервера DHCPv6 с отслеживанием состояния

Параметр DHCP-сервер с отслеживанием состояния требует, чтобы устройство с поддержкой IPv6 велело узлу связаться с сервером DHCPv6 для получения всех необходимых сведений о сетевой адресации IPv6.

На рисунке R1 будет предоставлять службы DHCPv6 с сохранением состояния для всех узлов в локальной сети. Настройка DHCPv6-сервера с отслеживанием состояния аналогична настройке сервера без такового. Наиболее значительным отличием является то, что сервер с отслеживанием состояния также является источником информации об IPv6-адресации, как и сервер DHCPv4.
...
На узле Windows используйте команду **ipconfig /all** для проверки метода DHCP без сохранения состояний. Выходные данные отображают настройки на PC1. Выделенные данные показывают, что PC1 получил свой IPv6 GUA от сервера DHCPv6 с сохранением состояния.
...
Роутер также может быть клиентом DHCPv6. Клиентское устройство должно включить **ipv6 unicast-routing** и локальный адрес канала IPv6 для отправки и получения сообщений IPv6.

Посмотрите пример топологии, чтобы узнать, как настроить клиент DHCPv6 с сохранением состояния.
...
**Шаг 1**. Включите маршрутизацию IPv6.   
**Шаг 2**. Настройте клиентский роутер для создания LLA.   
**Шаг 3**. Настройте клиентский роутер на использование DHCPv6.   
**Шаг 4**. Убедитесь, что клиентскому роутеру назначен GUA.   
**Шаг 5**. Убедитесь, что клиентский роутер получил другую необходимую информацию DHCPv6.  
...
В выходных данных **ipv6 enable** команда настраивается на интерфейсе R3 Gigabit Ethernet 0/0/1. Это позволяет роутеру создавать протокол LLA IPv6 без необходимости GUA.
...
Используйте команду **show ipv6 interface brief** для проверки конфигурации узла, как показано на иллюстрации.
...
Команда **show ipv6 dhcp interface g0/0/1** подтверждает, что DNS-имена и доменные имена также были изучены R3.
...
Используйте команды **show ipv6 dhcp pool** и **show ipv6 dhcp binding** для проверки работы DHCPv6 на роутере.

**show ipv6 dhcp pool**

Спомощью команды **show ipv6 dhcp pool** можно проверить имя DHCPv6-пула и его параметры. Она также определяет количество активных клиентов. В этом примере пул IPV6-STATEFUL в настоящее время имеет 2 клиента, что отражает получение PC1 и R3 их глобального одноадресного IPv6-адреса с этого сервера.

Когда роутер предоставляет службы DHCPv6 с сохранением состояния, он также поддерживает базу данных IPv6-назначенных.
...
<!-- 8.4.7 -->
## Настройка роутера в качестве агента ретрансляции DHCPv6

В случае если DHCPv6-сервер расположен с клиентом в разных сетях, в качестве агента DHCPv6-ретрансляции можно настроить роутер IPv6. Настройка агента DHCPv6-ретрансляции аналогична настройке IPv4-роутера в качестве агента DHCPv4-ретрансляции.
...
Синтаксис команды для настройки роутера в качестве агента ретрансляции DHCPv6 выглядит следующим образом:

```
Router(config-if)# ipv6 dhcp relay destination ipv6-address [interface-type interface-number]
```

Эта команда настраивается на интерфейсе, обращенном к клиентам DHCPv6, и указывает адрес сервера DHCPv6 и исходящий интерфейс для доступа к серверу, как показано в выходных данных. Исходящий интерфейс требуется только в том случае, если адрес следующего перехода является LLA.
