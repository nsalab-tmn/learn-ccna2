...
Часть 2. Создать сеть VLAN и назначить порты коммутатора

Часть 3. Поддержать назначение портов VLAN и базы данных VLAN

Часть 4. Настроить магистральный канал стандарта 802.1Q между коммутаторами

## Общие сведения/сценарий

Для повышения производительности сети большие широковещательные домены второго уровня делят на домены меньшего размера. Для этого современные коммутаторы используют виртуальные локальные сети (VLAN). Также VLAN можно использовать для определения узлов, между которыми возможен обмен данными, что позволяет повысить уровень безопасности. Эти сети облегчают процесс проектирования сети, обеспечивающей помощь в достижении целей организации.

Транковые каналы VLAN используются для распространения этих сетей по различным устройствам. Они разрешают передачу трафика из множества VLAN через один канал, не нанося вред идентификации и сегментации сети.

В этой лабораторной работе в режиме симмуляции физического оборудования вы создадите VLAN на обоих коммутаторах в топологии, назначите VLAN для коммутации портов доступа, убедитесь, что эти сети работают должным образом. Затем между двумя коммутаторами будет создана магистраль VLAN, чтобы разрешить узлам в одной сети взаимодействовать через нее, независимо от того, к какому коммутатору подключен узел.
...
3.  Обеспечьте сетевое подключение, используя кабели **Copper Straight-through**, как показано в топологии.

4.  Подключите **Console Cable** от устройства **PC-A** к **S1** и от **PC-B** к **S2**.
...
9.  Настройте на коммутаторе IP-адрес, указанный в таблице адресации для сети VLAN 1.
    ...
    **Примечание.** Адресация VLAN 1 не оценивается, так как позже вы ее удалите. Однако для проверки подключения позже в этой части потребуется VLAN 1.
...
В третьей части лабораторной работы вам предстоит изменить назначения сети VLAN портам и удалить сети из базы данных.
...
**Шаг 3. Удалите идентификатор VLAN из базы данных**
...
1.  Добавьте сеть VLAN 30 в интерфейс F0/24, не вводя команду сети VLAN.
    ...
    **Примечание.** Чтобы добавить сеть VLAN в базу данных, на современных коммутаторах больше не нужно выполнять команду **vlan**. Когда порту назначается неизвестная VLAN, она сразу добавляется в базу данных.

2.  Убедитесь, что новая сеть VLAN отображается в таблице.

    - Ответьте на вопрос №12

3.  Используйте команду **vlan 30**, чтобы удалить сеть VLAN 30 из базы данных.
...
6.  Выполните команду **show vlan brief**, чтобы определить назначение сети VLAN для F0/24.
    ...
    **Примечание.** Прежде чем удалять VLAN из базы данных, рекомендуется переназначить все порты, назначенные для этой сети.
...
По умолчанию протокол DTP на порте коммутатора 2960 настроен на динамический автоматический режим. Благодаря этому интерфейс может преобразовать канал в магистральный, если соседний интерфейс настроен на магистральный или динамический рекомендуемый режим.
...
1.  Настройте порт **F0/1** на коммутаторе **S1** для согласования магистрального режима.
    ...
    На коммутаторе **S2** должны быть получены сообщения о состоянии соединения.
...
2.  На коммутаторах **S1** и **S2** выполните команду **show vlan brief**. Интерфейс F0/1 больше не назначен VLAN 1. Транковые интерфейсы не указаны в таблице VLAN.

3.  Для просмотра магистральных интерфейсов выполните команду **show interfaces trunk**. Обратите внимание, что на коммутаторе **S1** настроен **desirable** (рекомендуемый) режим, а на **S2** – **auto** (автоматический).
...
Команда **switchport mode trunk** позволяет настроить порт в качестве магистрального канала вручную. Эту команду следует выполнять на обоих концах канала.
...
3.  Измените конфигурацию магистрали на обоих коммутаторах, изменив **native** (собственную) VLAN с VLAN 1 на VLAN 1000.
