...
Чтобы противостоять угрозам, препятствовать проникновению беспроводных хакеров и защитить данные, изначально использовались две ранние функции безопасности, которые по-прежнему доступны на большинстве роутеров и точек доступа: **маскировка SSID** и **фильтрация MAC-адресов**.

**Маскировка SSID** 

AP и некоторые беспроводные роутеры позволяют отключить кадр маяка SSID, как показано на рисунке. Клиенты должны вручную настроить SSID для подключения к сети.

![](./assets/12.7.2-1.png)

**Фильтрация MAC-адресов**

Администратор может вручную разрешить или запретить клиентам беспроводной доступ на основе их физического аппаратного MAC-адреса. На рисунке роутер настроен на разрешение двух MAC-адресов. Устройства с другими MAC-адресами не смогут подключиться к беспроводной сети 2,4 ГГц.
...
* **Аутентификация открытой системы.** Любой беспроводной клиент должен легко подключаться и пользоваться сетью только в ситуациях, когда безопасность не имеет значения. Например, когда предоставляется бесплатный доступ в Интернет в кафе, отелях и в удаленных областях. Беспроводной клиент отвечает за обеспечение безопасности, например, использование виртуальной частной сети (VPN) для безопасного подключения. Услуги аутентификации и шифрования VPN выходит за рамки этой темы.

* **Аутентификация с помощью общего ключа.** Предоставляет механизмы аутентификации и шифрования данных, передаваемых между беспроводным клиентом и точкой доступа или роутером. Однако для подключения необходимо предварительно согласовать пароль между сторонами.
...
| --- | --- |
| **Протокол безопасности, аналогичный защите проводной сети (Wired Equivalent Privacy, WEP)** | Первоначальная спецификация 802.11 предназначена для защиты данных с использованием метода шифрования Rivest Cipher 4 (RC4) со статическим ключом. Однако ключ никогда не меняется при обмене пакетами. Это позволяет легко взломать сеть. WEP больше не рекомендуется и никогда не должен использоваться |
...
| **WPA3** | Следующее поколение безопасности Wi-Fi. Все устройства с поддержкой WPA3 используют новейшие методы обеспечения безопасности, запрещают устаревшие протоколы и требуют использования защищенных кадров управления (PMF). Однако устройства с WPA3 пока недоступны |

<!-- 12.7.5 -->
## Аутентификация домашнего пользователя

Домашние роутеры обычно имеют два варианта аутентификации: WPA и WPA2 (сильнейший из них). На рисунке показана возможность выбора одного из двух методов аутентификации WPA2.

* **Персональный** предназначен для домашней или небольшой офисной сети, пользователи проходят аутентификацию с использованием предварительного ключа (PSK). Беспроводные клиенты проходят проверку подлинности с помощью роутера с использованием предварительного общего пароля. Специализированный сервер аутентификации не требуется.
* **Корпоративный** предназначен для корпоративных сетей, но для него требуется сервер удаленной аутентификации (RADIUS). Хотя этот тип более сложен для настройки, он обеспечивает повышенную безопасность. Устройство должно выполнить аутентификацию посредством сервера RADIUS, после чего пользователи должны пройти ее, используя стандарт 802.1X, который задействует усовершенствованный протокол — EAP.

На рисунке администратор настраивает беспроводной роутер с персональной аутентификацией WPA2 в диапазоне 2,4 ГГц.
...
* **расширенный стандарт шифрования (AES)** (метод шифрования, используемый WPA2, который является предпочтительным, потому что гораздо более надежен и использует режим счетного шифра с протоколом кода аутентификации сообщений с цепочкой блоков (CCMP), позволяющим узлам назначения распознавать, были ли изменены зашифрованные и незашифрованные биты).

На рисунке администратор настраивает беспроводной роутер для использования WPA2 с шифрованием AES в диапазоне 2,4 ГГц.
...
На рисунке администратор настраивает беспроводной роутер с аутентификацией WPA2 Enterprise с использованием шифрования AES. IPv4-адрес сервера RADIUS также настроен с использованием надежного пароля между роутером и сервером RADIUS.
...
На момент написания этой статьи, устройства, поддерживающие аутентификацию WPA3, были недоступны. Однако WPA2 больше не считается безопасным. WPA3, если он доступен, является рекомендуемым методом аутентификации 802.11. Он включает в себя четыре функции:
...
В **WPA3-Personal** субъекты угроз могут прослушивать контакт между беспроводным клиентом и точкой доступа и использовать атаку методом грубой силы, чтобы попытаться угадать PSK. WPA3-Personal предотвращает эту атаку с помощью одновременной аутентификации равных (SAE), функции, указанной в IEEE 802.11-2016. PSK никогда не разоблачается, злоумышленнику невозможно его угадать.
