...
В предыдущем разделе были перечислены три различных способа создания маршрутизации между VLAN, а также подробно описан устаревший метод. В этом разделе детально рассмотрим, как настроить маршрутизацию между виртуальными локальными сетями на роутере. Вы можете видеть на рисунке, что устройство находится не в центре топологии, а вместо этого расположено рядом с границей (на флешке). Отсюда и название – Router-on-a-stick (ROS).
